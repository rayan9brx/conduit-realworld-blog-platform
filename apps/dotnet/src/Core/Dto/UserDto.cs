...
public record UpdatedUserDto
    (string? Username, string? Email, string? Bio, string? Image, string? OldPassword, string? NewPassword) : IValidatableObject
{
    public IEnumerable<ValidationResult> Validate(ValidationContext validationContext)
    {
        if (string.IsNullOrWhiteSpace(Username) && string.IsNullOrWhiteSpace(Email) &&
            string.IsNullOrWhiteSpace(Bio) && string.IsNullOrWhiteSpace(Image) && string.IsNullOrWhiteSpace(OldPassword)
            && string.IsNullOrWhiteSpace(NewPassword))
        {
            yield return new ValidationResult(
                $"At least on of the fields: {nameof(Username)}, {nameof(Email)}, {nameof(Bio)}, {nameof(Image)}, {nameof(OldPassword)}, {nameof(NewPassword)} must be filled"
            );
        }
    }
}
